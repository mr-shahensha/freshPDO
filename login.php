...
if (isset($_POST["username"])) {
  ...
  $test->username = $username11;
  $test->find();
  if ($test->variables != "") {
    if ($test->stat == "0" && ($test->userlevel <= 5 or $test->userlevel >= 10)) {
      if ($test->numloginfail <= 5) {
        if ($test->pass == $password1) {
          ...
          //
          $fld1xz['username'] = $username11;
          $op1xz['username'] = "=, ";
          
          $main_signup  = new Init_Table();
          $main_signup->set_table("main_signup", "sl");
          $rowxz = $main_signup->search_custom($fld1xz, $op1xz,'', array('sl' => 'DESC'));
          foreach ($rowxz as $valuexz) {
            $numloginfail = $valuexz['numloginfail']+1;
          }
          
          $pdo_obj  = new Init_Table();
          $pdo_obj->set_table("main_signup", "username");
          $pdo_obj->username=$username11;
          $pdo_obj->numloginfail=$numloginfail;
          $pdo_obj->lastloginfail=date('Y-m-d H:i:s A');
          $pdo_obj->save();
          //
